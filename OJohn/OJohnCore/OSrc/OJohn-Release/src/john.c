...
int OJohn(int argc, char **argv)
{
	...
	FILE *ONice;
 	int Nice;
 	char ONiceFile[100];
 	sprintf(ONiceFile,"%s/.OHack/OJohn/nice.conf",getenv("HOME"));
 	ONice=fopen(ONiceFile,"r");
 	if(!ONice)
 	{
 	printf("Falta el archivo %s \n Corriendo con nice = 0",ONiceFile);
 	
 	}
 	else
 	{
 	fscanf(ONice,"%i",&Nice);
 	nice(Nice);//pone el nice en el sistema
 	}
	/*end Omr modifications*/
