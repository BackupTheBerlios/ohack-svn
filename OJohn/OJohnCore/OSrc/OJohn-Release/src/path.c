...
{
	if (!strncmp(name, "~/.OHack/OJohn", 14)) {
		if (john_home_path &&
		    john_home_length + strlen(name) - 14 < PATH_BUFFER_SIZE) {
			strnzcpy(&john_home_path[john_home_length], &name[14],
	...
	if (!strncmp(name, "~/.OHack/OJohn", 14)) {
		if (user_home_path &&
		    user_home_length + strlen(name) - 14 < PATH_BUFFER_SIZE) {
			strnzcpy(&user_home_path[user_home_length], &name[14],
